...
class PlantViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not last_watered_date:
            return Response(
                {"detail": "Last watered date is required."},
        ...
        if last_watered_date > datetime.now().date():
            return Response(
                {"detail": "Last watered date cannot be in the future."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {"detail": "Last watered date updated successfully."},
            status=status.HTTP_200_OK,
        )

    def destroy(self, request, *args, **kwargs):
        return Response(
            {"detail": "Method not allowed"}, status=status.HTTP_400_BAD_REQUEST
        )
